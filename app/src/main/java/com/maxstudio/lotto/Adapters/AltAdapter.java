...
public class AltAdapter extends RecyclerView.Adapter<RecyclerView.ViewHolder>{
    ...
    public AltAdapter(Context context, List<DailyResultsPicker> combinedDataList, DatabaseReference
    ...
    public static class DailyResultsViewHolder extends RecyclerView.ViewHolder {
        TextView Lotto1, Lotto2, Lotto3, Lotto4, Lotto5, day, Mach2, Mach3,
        ...
        public DailyResultsViewHolder(@NonNull View itemView) {
            ...
            LottoImageMon = itemView.findViewById(R.id.daywa);
            LottoImageTues = itemView.findViewById(R.id.vag);
    ...
    public void onBindViewHolder(@NonNull RecyclerView.ViewHolder holder, int position) {
        ...
        if (getItemViewType(position) == VIEW_TYPE_FIRST) {
            ...
            int positions = position - (position > 0 ? 1 : 0) - (position > 4 ? 1 : 0) - (position > 7 ? 1 : 0);

            if (DaywaFragment.isRefreshed && VagFragment.isRefreshed)
            {
                        ...
                        public void onDataChange(@NonNull DataSnapshot dataSnapshot)
                        {
                            if(dataSnapshot.exists())
                            {
                                if (dataSnapshot.hasChild("lotto1"))
                                {
                                    ...
                                    DaywaFragment.isRefreshed = false;
                                    VagFragment.isRefreshed = false;

                                    String image = dataSnapshot.child("draw_type").getValue().toString();

                                    if (image.equals("Daywa"))
                                    {
                                        dailyResultsViewHolder.LottoImageMon.setVisibility(View.VISIBLE);
                                    }
                                    if (image.equals("Vag"))
                                    {
                                        dailyResultsViewHolder.LottoImageTues.setVisibility(View.VISIBLE);
                                    }
